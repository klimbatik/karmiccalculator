...
        // === РОДОВЫЕ БЛОКИ ===
        let html = '<h3>РОДОВЫЕ БЛОКИ</h3>';

        if (missing.includes(7)) {
            html += `
                <p><strong>Суицид в роду.</strong><br>
...
                Вы пришли с задачей прервать этот цикл через доверие жизни, не бояться одиночества, не искать "конца" как избавления. 
                Каждый раз, когда вы выбираете жизнь — вы исцеляете род.</p>
            `;
        }

        if (missing.includes(2)) {
            html += `
                <p><strong>Физическая агрессия и потеря энергии.</strong><br>
...
                Сила — не в крике, а в молчании. Не в агрессии, а в спокойствии.</p>
            `;
        }

        if (missing.includes(4)) {
            html += `
                <p><strong>Карма тела и здоровья.</strong><br>
...
                Каждый раз, когда вы выбираете здоровье — вы исцеляете прошлую боль.</p>
            `;
        }

        if (karmicNumbers.includes(14)) {
            html += `
                <p><strong>Зависимости в роду.</strong><br>
...
                Настоящая сила — в способности быть одной и быть в покое.</p>
            `;
        }

        if (karmicNumbers.includes(16)) {
            html += `
                <p><strong>Физическая и эмоциональная агрессия.</strong><br>
...
                Но вы можете её проработать: через прощение, через осознание, через отказ от контроля.</p>
            `;
        }

        if (digits.includes(6) && karmicNumbers.includes(19)) {
            html += `
                <p><strong>Привороты и магия в любви.</strong><br>
...
                Пусть человек остаётся — не из-за магии, а из-за любви.</p>
            `;
        }

        if (missing.includes(8)) {
            html += `
                <p><strong>Долг перед родом.</strong><br>
...
                Род исцеляется не через жертву, а через радость.</p>
            `;
        }

        if (html === '<h3>РОДОВЫЕ БЛОКИ</h3>') {
            html += `<p>По вашей дате рождения не выявлено явных родовых блоков. Это хороший знак — вы пришли не для проработки тяжёлых программ, а для развития своих сильных сторон.</p>`;
        }
...
        if (lifePath === 6) {
            html += `
                <p><strong>Хранительница семьи.</strong><br>
...
                Ваша сила — не в жертвенности, а в способности любить свободно.</p>
            `;
        }

        if (karmicNumbers.includes(19)) {
            html += `
                <p><strong>От власти к служению.</strong><br>
...
                Ваша миссия — быть не властителем, а проводником света.</p>
            `;
        }

        if (karmicNumbers.includes(22)) {
            html += `
                <p><strong>Мастер, который не тиран.</strong><br>
...
                Истинный мастер — тот, кто освобождает, а не привязывает.</p>
            `;
        }

        if (missing.includes(5)) {
            html += `
                <p><strong>От интуиции к разуму.</strong><br>
...
                Не бойтесь думать. Разум — не враг интуиции, а её союзник.</p>
            `;
        }

        if (!html.includes('ПРОГРАММЫ ДУШИ</h3><p>')) {
            html += `<p>Ваша душа пришла с сильными программами роста. Вы несёте в себе потенциал для глубокого развития и исцеления. Продолжайте следовать своему пути — он ведёт к свету.</p>`;
        }

        // Показываем результат
        resultDiv.innerHTML = html;
        resultDiv.style.display = "block";
        resultDiv.scrollTop = 0; // Прокрутка в начало
    });
});